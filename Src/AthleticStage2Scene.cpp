...
* シーンの初期化
*/
bool AthleticStage2Scene::Initialize(Engine& engine)
{
	//環境光の設定：全体的に明るくしてある
	...
	gameObjectSettingsjson->AthleticStage2MapLoad();
	
	// プレイヤーを配置
	auto player = engine.Create<GameObject>("player", "player", { 0,10.0f, 0.0f },{0,0.0f,0});
	...
	auto meScene =
		engine.Create<GameObject>("exploration", "exploration", { 26.0f, 13.8f, 17.0f });
	moveExplorationScene = meScene->AddComponent<MoveExplorationScene>();

	//ステージに遷移用のオブジェクト
	auto ms3Scene =
		engine.Create<GameObject>("Stage3", "Stage3", { 22.0f, 13.8f, 17.0f });
	moveStage3Scene = ms3Scene->AddComponent<MoveStage3Scene>();

	//ゴールの直前に設置しているゴールっぽく演出しているオブジェクト
	auto tornado =
		engine.Create<GameObject>("tornado", "tornado", { 26.3f, 13.8f, 17.0f });
	tornadoComponent = tornado->AddComponent<TornadoEffect>();
	auto tornado1 =
		engine.Create<GameObject>("tornado", "tornado", { 22.3f, 13.8f, 17.0f });
	tornadoComponent1 = tornado1->AddComponent<TornadoEffect>();

	// ネクストシーンの説明の表示
	auto textBlock =
		engine.Create<GameObject>("next", "next", { 26.7f, 13.8f, 17.0f });
	textBlockComponent = textBlock->AddComponent<TextBlock>();
	// シーンを戻るときの説明の表示
	auto textBlock1 =
		engine.Create<GameObject>("back", "back", { 22.7f, 13.8f, 17.0f });
...
void AthleticStage2Scene::Update(Engine& engine, float deltaTime)
{
...
// 状態に対応する更新関数を実行
	...
	if (fadeTimer > 0) {
		...
		else if (fadeTimer <= 0.0f && isAthleticStage3)
		{
...
void AthleticStage2Scene::Finalize(Engine& engine)
{
	//ここで生成したオブジェクトの削除
...
void AthleticStage2Scene::State_Playing(Engine& engine, float deltaTime)
{
	//初期設定
	...
	auto ms3Scene = moveStage3Scene->GetOwner();
	...
	//AthleticScene3に移動するときに画像をプレイヤーが近づいたら出してくれるようにしている
	...
	if (textBlockComponent1->IsPlayerInRange(player->position.x, player->position.z, 1.5f))
		textBlockComponent1->SetIsHitPlayer4(true);
	else
		textBlockComponent1->SetIsHitPlayer4(false);
		...
		if (fadeTimer <= 0)
		{
			...
			retryButton.second->onClick.push_back(
				[this](UIButton* button) {
					...
					isAthleticStage2 = true;	  //もう一度始める際のフラグをOnに
	...
	*/
	...
	//Stage3に進む時のif文
	if (moveStage3Scene->GetState() == MoveStage3Scene::MoveStage3SceneState::Inside)
	{
		//プレイヤーをアスレチックシーンに遷移させる
		if (fadeTimer <= 0) {
			fadeTimer = 1.0f;		//シーン遷移の処理を始める
			isAthleticStage3 = true;//AthleticStage3に飛ばすフラグをOnに
			// フェードアウト用UIオブジェクト
			// 画面全体を覆うサイズに設定
			auto fade = engine.CreateUIObject<UILayout>(Image_Fade::white, "fade", { 0, 0 }, 1);
			FadeObject = fade.first;
...
void AthleticStage2Scene::State_GameOver(Engine& engine, float deltaTime)
{
	//先にplayerを削除しておく
	auto player = playerComponent->GetOwner();
	player->Destroy();
